...
- $\log \left(\delta\omega_\mathrm{rot, g} / \mathrm{\mu Hz}\right)$ --- the implementation of the PSD model (below) assumes no envelope rotation, but in principle the generative model for mode frequencies also accepts an envelope rotation rate.
